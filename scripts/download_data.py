...
async def main():
    ...
    console.print(f"📦 Downloading data for subject [bold]{subject.subjectIdentifier}[/bold] (ID: {subject.id})")
    ...
    with progress:
        ...
        for f in files.items:
            ...
            tasks.append(
                download_gz_file(f.downloadUrl, output_path, sem, progress, task_id)
            )
        await asyncio.gather(*tasks)  # ← This line was out of place in your version
